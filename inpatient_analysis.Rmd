...
# ── 7. Add variable labels for SPSS export ──────────────────────────────────
...
for (i in 1:10) {
  var_label(inpt_claims[[paste0("icd9_dgns_cd_", i)]]) <- glue::glue("Diagnosis code {i} (ICD-9)")
}

# ICD-9 fields (procedures)
for (i in 1:6) {
  var_label(inpt_claims[[paste0("icd9_prcdr_cd_", i)]]) <- glue::glue("Procedure code {i} (ICD-9)")
}
...
# ── 8. Add value labels for SPSS export (requires integer coding) ───────────

inpt_claims_export <- inpt_claims %>%
  mutate(
...
val_labels(inpt_claims_export$tob_first) <- c(
  "Inpatient (hospital)" = 1,
  "Outpatient"           = 2,
  "Other"                = 3
)

val_labels(inpt_claims_export$tob_last) <- c(
...
val_labels(inpt_claims_export$tob_rank) <- c(
  "Replacement claim" = 1,
  "Final claim"       = 2,
  "Interim/Void"      = 3
)
...
write_sav(inpt_claims_export, "data/inpatient_claims_clean_export.sav")
